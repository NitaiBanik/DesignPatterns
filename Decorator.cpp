...
public:
  BackpackDecorator(IBackpack *decorator) { _decorator = decorator; }
  virtual void assemble() { _decorator->assemble(); }
};

class LaptopBackpackDecorator : public BackpackDecorator {
public:
  LaptopBackpackDecorator(IBackpack *decorator)
      : BackpackDecorator(decorator) {}
